"""PLAN_HERMES — integración del modelo del agente configurable (FASE 5).

Hermes 3:3b se evaluó y se descartó como router (1/12 en la batería, no emite
`tool_calls` fiables; ver `docs/AUDITORIA_2026-09.md` §7). Lo que queda y se
prueba aquí es la infraestructura que sirve para CUALQUIER modelo:

- override del modelo del agente / chat por variable de entorno.
- `jarvis doctor` avisa si el modelo de routing no soporta tool calling.
"""
import os
import sys
...
def test_sin_env_usa_config_yaml(monkeypatch):
    ...
    cfg = cfg_module.reload_config()
    # los valores de config.yaml (o el default) siguen mandando
...
# ── FASE 5: doctor comprueba capacidad de tool calling ─────────────────────
...
def test_doctor_router_sin_tools_es_falta():
    with patch("jarvis_local.ollama_client.client.OllamaClient") as C, \
         patch("jarvis_local.config.get_config",
               return_value={"ollama": {"host": "h", "model": "qwen2.5:3b",
                                        "agent_model": "modelo-raro"}}):
        C.return_value = _client_mock(
            ["qwen2.5:3b", "modelo-raro", "bge-m3"], ["completion"])
...
def test_doctor_router_con_tools_ok():
    with patch("jarvis_local.ollama_client.client.OllamaClient") as C, \
        ...
        C.return_value = _client_mock(
            ["qwen2.5:3b", "llama3.2:3b", "bge-m3"], ["completion", "tools"])
        lineas = doctor._check_ollama()
...
def test_doctor_capacidad_desconocida_no_rompe():
    ...
    with patch("jarvis_local.ollama_client.client.OllamaClient") as C, \
         patch("jarvis_local.config.get_config",
               return_value={"ollama": {"host": "h", "model": "qwen2.5:3b",
                                        "agent_model": "llama3.2:3b"}}):
        c = _client_mock(["qwen2.5:3b", "llama3.2:3b", "bge-m3"], None)
